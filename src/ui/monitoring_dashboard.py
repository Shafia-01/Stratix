                ...
                if jobs:
                    for job in jobs:
                        with st.container():
                            ...
                            with col4:
                                if st.button("🗑️", key=f"del_job_{job['id']}", help="Delete monitoring job"):
                                    del_resp = requests.delete(f"{API_BASE_URL}/monitor/{job['id']}")
    ...
    with tabs[1]:
        ...
        try:
            ...
            if hist_resp.status_code == 200:
                ...
                if not history:
                    st.info("No monitoring execution runs found yet.")
                else:
    ...
    with tabs[2]:
        ...
        try:
            ...
            if all_jobs_resp.status_code == 200:
                history_data = all_jobs_resp.json()
                if history_data:
                    ...
                    if trends_resp.status_code == 200:
                        ...
                        if not trends:
                            ...
                            fig = px.line(
                                df_trends,
                                x="run_id",
                                y=["plan_quality", "report_quality", "tool_reliability"],
                            ...
                            for run_eval in trends:
                                with st.expander(f"Run ID: {run_eval['run_id'][:12]} — Date: {run_eval.get('created_at', 'Unknown')}"):
                                    st.markdown(f"**Plan Quality Score:** `{run_eval['plan_quality']}/1.0`")
                                    st.markdown(f"**Rationale:** {run_eval.get('plan_rationale')}")
                                    st.markdown("---")
                                    st.markdown(f"**Report Quality Score:** `{run_eval['report_quality']}/1.0`")
                                    st.markdown(f"**Rationale:** {run_eval.get('report_rationale')}")
                                    st.markdown("---")
                                    st.markdown(f"**Tool Reliability Score:** `{run_eval['tool_reliability']}/1.0`")
                                    st.markdown(f"**Rationale:** {run_eval.get('tool_reliability_rationale')}")
                else:
                    st.info("No runs found to evaluate yet.")
            else:
                st.error("Failed to load historical runs for evaluation analytics.")
        except Exception as e:
            st.error(f"Backend API connection failed: {e}")
